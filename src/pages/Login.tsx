    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">InstaCreator</h1>
          <p className="text-sm text-muted-foreground mt-1">Crea contenido que marca tendencia</p>
        </div>

        <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
          <h2 className="text-lg font-bold text-foreground mb-5">Iniciar sesión</h2>
          <div className="space-y-4">
            ...
            <button className="w-full gradient-brand text-primary-foreground py-3 rounded-xl font-semibold text-sm shadow-brand hover:opacity-90 transition-opacity">Entrar</button>
          </div>
        ...
        <p className="text-center text-sm text-muted-foreground mt-5">
          ¿No tienes cuenta?{' '}
          <Link to="/signup" className="text-primary font-semibold">Regístrate</Link>
        </p>
      </motion.div>
    </div>
  );
